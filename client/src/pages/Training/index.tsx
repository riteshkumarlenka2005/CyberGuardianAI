...
const Training: React.FC = () => {
  ...
  const saveAndExitSession = async () => {
    if (selectedScenario && selectedIdentity && sessionStartTime > 0) {
      ...
      await UserDataService.saveSession({
  ...
  // --- ACTIVE CHAT UI ---
  return (
    <div className="pt-20 min-h-screen bg-slate-100 dark:bg-[#050914] flex flex-col lg:flex-row overflow-hidden font-sans">

      {/* LEFT SIDEBAR (Console Style - Hidden on Mobile) */}
      <aside className="hidden lg:flex w-72 bg-white dark:bg-[#0B1221] border-r border-slate-200 dark:border-slate-800/50 flex flex-col relative z-20 shadow-xl">
      ...
      {/* CENTER STAGE */}
      <div className="flex-grow flex flex-col relative h-[calc(100vh-5rem)] overflow-hidden">

        {/* HEADER BAR (Purple Mecha Style) */}
        <div className="h-16 bg-white dark:bg-[#0B1221] border-b border-slate-200 dark:border-purple-900/30 flex items-center justify-between px-4 sm:px-8 z-30 shadow-md">
          <div className="flex items-center gap-3 sm:gap-4 overflow-hidden">
            <div className="flex-shrink-0 w-8 h-8 bg-purple-100 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 flex items-center justify-center rounded-lg border border-purple-200 dark:border-purple-500/30">
              {SCENARIOS.find(s => s.id === selectedScenario)?.icon}
            </div>
            <div className="overflow-hidden">
              <h2 className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider truncate">{SCENARIOS.find(s => s.id === selectedScenario)?.title}</h2>
              <p className="text-[8px] sm:text-[10px] text-purple-500 font-mono truncate">ENCRYPTION: AES-256 // SIMULATION</p>
            </div>
          </div>
          <button onClick={saveAndExitSession} className="flex-shrink-0 px-3 sm:px-4 py-1.5 border border-red-200 dark:border-red-900/50 text-red-500 text-[8px] sm:text-[10px] font-bold uppercase tracking-widest hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
            Abort
          </button>
        </div>
        ...
        <div className="flex-grow flex flex-col sm:flex-row relative overflow-hidden">
          ...
          {/* Messages */}
          <div ref={scrollRef} className={`flex-grow overflow-y-auto p-4 sm:p-8 space-y-6 sm:space-y-8 bg-slate-100 dark:bg-[#050914] transition-all duration-500 ${showMentor ? 'sm:w-1/2 hidden sm:block' : 'w-full'}`}>
            {messages.map((msg) => (
              <MessageBubble key={msg.id} message={msg} />
            ))}
            {isLoading && (
              <div className="flex items-center gap-2 text-[10px] sm:text-xs font-mono text-slate-400 pl-4 animate-pulse">
          ...
          {/* MENTOR PANEL (Slide-in / Overlay on mobile) */}
          {showMentor && risk && (
            <div className="w-full sm:w-1/2 bg-white dark:bg-[#0B1221] border-l border-slate-200 dark:border-slate-800 flex flex-col relative shadow-2xl animate-slide-in-right z-20 h-full">
              {/* Pink/Yellow Gradient Top Border */}
              <div className="h-1 w-full bg-gradient-to-r from-pink-500 via-orange-500 to-yellow-500"></div>

              <div className="p-4 sm:p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-white/5">
                <div className="flex items-center gap-3">
                  <div className="w-7 h-7 sm:w-8 sm:h-8 bg-pink-500 text-white flex items-center justify-center font-bold text-base sm:text-lg rounded shadow-lg shadow-pink-500/30">!</div>
                  <h3 className="font-black text-slate-900 dark:text-white uppercase tracking-widest text-xs sm:text-sm">Intervention</h3>
                </div>
                <div className="text-[8px] sm:text-[10px] font-mono text-pink-500 animate-pulse">THREAT DETECTED</div>
              </div>

              <div className="flex-grow overflow-y-auto p-5 sm:p-8 space-y-6 sm:space-y-8">
                <div className="p-4 sm:p-6 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg relative overflow-hidden">
                  <div className="absolute left-0 top-0 bottom-0 w-1 bg-pink-500"></div>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 sm:mb-3">Analysis</p>
                  <p className="text-sm sm:text-base text-slate-700 dark:text-slate-300 font-medium leading-relaxed">{risk.explanation}</p>
                </div>

                <div className="grid grid-cols-2 gap-3 sm:gap-4">
                  <div className="p-3 sm:p-4 border border-orange-200 dark:border-orange-900/30 bg-orange-50 dark:bg-orange-900/10 rounded">
                    <p className="text-[8px] sm:text-[10px] font-bold text-orange-500 uppercase tracking-widest mb-1">Vector</p>
                    <p className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white">{risk.manipulationTactic}</p>
                  </div>
                  <div className="p-3 sm:p-4 border border-red-200 dark:border-red-900/30 bg-red-50 dark:bg-red-900/10 rounded">
                    <p className="text-[8px] sm:text-[10px] font-bold text-red-500 uppercase tracking-widest mb-1">Level</p>
                    <p className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white uppercase">Critical</p>
                  </div>
                </div>

                <div className="p-5 sm:p-6 bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 rounded-lg">
                  <p className="text-[9px] sm:text-[10px] font-bold text-blue-500 uppercase tracking-widest mb-2 sm:mb-3">Protocol</p>
                  <p className="text-sm sm:text-base text-blue-800 dark:text-blue-200 font-medium italic">"{risk.guidance}"</p>
                </div>
              </div>

              <div className="p-4 sm:p-6 border-t border-slate-200 dark:border-slate-800 flex gap-3 sm:gap-4 bg-slate-50 dark:bg-[#080c17]">
        ...
        <div className="p-4 sm:p-6 bg-white dark:bg-[#0B1221] border-t border-slate-200 dark:border-slate-800 z-30">
          <div className="max-w-4xl mx-auto relative group">
            {/* Frame Brackets (Hidden on very small screens to save space) */}
            <div className="hidden xs:block absolute -top-2 -left-2 w-4 h-4 border-t-2 border-l-2 border-slate-300 dark:border-cyan-500/50 group-focus-within:border-cyan-500 transition-colors"></div>
            <div className="hidden xs:block absolute -bottom-2 -right-2 w-4 h-4 border-b-2 border-r-2 border-slate-300 dark:border-cyan-500/50 group-focus-within:border-cyan-500 transition-colors"></div>
            ...
            <form onSubmit={handleSendMessage} className="relative flex items-center gap-2">
              <input
                ...
                placeholder={showMentor ? "DECIDING..." : "MESSAGE..."}
                className="flex-grow bg-slate-100 dark:bg-[#050914] border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white px-4 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm font-mono focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all outline-none disabled:opacity-50 disabled:cursor-not-allowed"
              />
              <button
                type="submit"
                disabled={showMentor || isLoading}
                className="px-4 sm:px-8 py-3 sm:py-4 bg-slate-900 dark:bg-cyan-600 text-white font-bold text-[10px] sm:text-xs uppercase tracking-widest hover:bg-slate-800 dark:hover:bg-cyan-700 disabled:bg-slate-300 dark:disabled:bg-slate-800 transition-all flex-shrink-0"
              >
                Send
              </button>
            </form>
